...
Fill in "Files to archive" :  build/libs/citytest*.jar
Click "Advanced..."
...
* Project name: citytest-<your_name>-build
* Which build: Specified by a build parameter
*    Parameter Name: BUILD_SELECTOR

. Under +Build+, add a +Execute Shell+ build step. Remember to replace in the script below +<your_name>+ with your name without space!
+
Command::
+
[source,bash]
...
cf push "cities-${YOUR_NAME}-$BUILD_VERSION" -i 1 -m 512M -n "cities-${YOUR_NAME}-$ROUTE_VERSION" -d ${PCF_APP_DOMAIN} -p artifacts/citytest-${BUILD_VERSION}.jar --no-manifest
